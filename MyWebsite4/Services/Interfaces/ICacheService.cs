...
namespace MyWebsite4.Services
{
    public interface ICacheService
    {
        ...
        /// <param name="expiressAbsoulte">绝对过期时长</param>
        /// <returns></returns>
        Task<bool> SetAsync(string key, object value, TimeSpan expiresSliding, TimeSpan expiressAbsoulte);
        ...
        /// <param name="isSliding">是否滑动过期（如果在过期时间内有操作，则以当前时间点延长过期时间）</param>
        /// <returns></returns>
        Task<bool> SetAsync(string key, object value, TimeSpan expiresIn, bool isSliding = false);
        #endregion
    }
}
